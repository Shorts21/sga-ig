...
async function run() {
    ...
    if (pivosErr) console.error("Error fetching pivos:", pivosErr);
    if (error) console.error("Error fetching pivosGeo:", error);
    ...
    if (pivosGeo) {
        for (const geo of pivosGeo) {
            if (!existingNames.has(geo.nome)) {
                newPivos.push({
                    nome: geo.nome,
                    area_hectares: geo.area_ha || 0
                });
            }
        }

        if (newPivos.length > 0) {
            console.log('Inserting into pivos:', newPivos.length);
