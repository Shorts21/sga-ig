-- 1. Create 'poligonais' table if it doesn't exist
...
-- 2. Create 'pivos' table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.pivos (
    ...
    trabalhadores_atual INTEGER,
    position JSONB,
    created_at TIMESTAMPTZ DEFAULT now()
);

-- 3. Create 'colaboradores' table
...
-- 4. Create RPC function to update pivot summaries
...
BEGIN
    ...
    WITH summary AS (
        SELECT
            ...
            COALESCE(SUM(c.salario), 0) as calculated_cost
        FROM
            public.pivos p
        LEFT JOIN
